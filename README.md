...
| --- | --- | --- |
| `apps/web` | Aplicação React + TypeScript: 17 telas, 34 formulários de escrita, simulador comercial, mapa geográfico com camada raster e busca de endereço, leitor de XML da NF-e, árvore de permissões módulo → tela → ação, tela de entrada com recusa uniforme, fila de aprovação de pagamentos com prévia de alçada, diálogo acessível, combobox, RBAC e base de teste do domínio de locação de TI | `npm run a11y:dom` — 165 testes de axe, teclado, formulários, permissões, entrada fiscal, mapa, política comercial, autenticação, centro de custo, conta bancária, contas a pagar e domínio · `npm run web:test` — 123 unitários da árvore de permissões, dos comandos de usuário e autenticação, do saldo derivado e da transferência, das nove regras de contas a pagar, da aritmética de tiles, do parser de geocodificação e da entrega de arquivos |
| `apps/api` | API NestJS sobre PostgreSQL com RLS: contexto de tenant por transação, autorização negada por padrão, `problem+json`, idempotência, concorrência otimista, a entrada fiscal de compra, os locais de operação, a autenticação com Argon2id, o worker de notificação com adaptador SMTP e as contas a pagar com aprovação por alçada | `npm run api:test` — 142 testes contra PostgreSQL real · `verificar-rotas.mjs` — 55/55 rotas declaram autorização explícita, e o build reprova quem esquecer |
| `packages/contracts` | Esquemas Zod compartilhados entre API e clientes: primitivos, catálogo de erros, catálogo das 123 permissões, a árvore módulo → tela → ação derivada dele, entidades e a chave de acesso da NF-e com dígito verificador | Compilado no CI; consumido pelos dois lados — o front deixou de ter vocabulário próprio |
| `packages/db` | 19 migrações SQL: fundação, identidade, auditoria, equipamentos, contratos, RLS, outbox, geoespacial, idempotência, nota fiscal de compra, eixo de cliente, franquia e preço, consumo, proveniência da coordenada, autenticação, superfície fechada de autenticação, centro de custo e conta bancária, notificação e contas a pagar | `npm run db:test` — 127 assertivas de invariante contra PostgreSQL real, com e sem PostGIS |
