...
| [R — Base do financeiro](docs/anexos/R-base-do-financeiro.md) | Módulos 8 e 9 implementados: saldo derivado sem coluna de saldo e por que isso importa, transferência como dupla entrada numa chamada só, movimentação imutável com as duas exceções justificadas, profundidade e ciclo na mesma travessia, D-16 e D-17 fechadas com o raciocínio, e os três defeitos que os testes acharam |
| [T — Contas a receber](docs/anexos/T-contas-a-receber.md) | Módulo 11 implementado: D-20 fechada com uma tabela só, o fechamento de competência que o levantamento dizia existir e não existia, o piso de um nível que faz a alçada decidir *quantos* conferem e não *se* alguém confere, `BAIXADO` que não é `RECEBIDO` e por que essa confusão não tem sintoma, o eixo de cliente na RLS com a pré-cobrança fora, e os nove defeitos que os testes acharam |
| [U — Lançamentos futuros e fluxo de caixa](docs/anexos/U-lancamentos-e-fluxo-de-caixa.md) | Módulos 12 e 13 implementados: a projeção especificada duas vezes e construída uma, a recorrência que é uma tabela e não duas, o id polimórfico que virou duas chaves estrangeiras reais depois de a verificação derrubar o precedente citado, a conversão que ocorre uma vez e a porta sequencial que o `for update` não fecha, `BAIXADO` que parece receita numa projeção de caixa, o cenário de estresse que não desconta a própria dívida, D-23 fechada, e os seis defeitos que a revisão e os testes acharam |
| [S — Contas a pagar](docs/anexos/S-contas-a-pagar.md) | Módulo 10 implementado: a segregação de funções que faz o resto significar algo, alçada configurável com o posto como posição e não como valor, delegação rastreável no lugar da credencial emprestada, o parcelamento como um compromisso e não N, a baixa e o extrato numa chamada só, a prévia de alçada antes de salvar, D-18 fechada, e os sete defeitos que os testes acharam — inclusive as parcelas que nasciam impagáveis |
...
| --- | --- | --- |
| `apps/web` | Aplicação React + TypeScript: 20 telas, 44 formulários de escrita, simulador comercial, mapa geográfico com camada raster e busca de endereço, leitor de XML da NF-e, árvore de permissões módulo → tela → ação, tela de entrada com recusa uniforme, fila de aprovação de pagamentos com prévia de alçada, diálogo acessível, combobox, RBAC e base de teste do domínio de locação de TI | `npm run a11y:dom` — 207 testes de axe, teclado, formulários, permissões, entrada fiscal, mapa, política comercial, autenticação, centro de custo, conta bancária, contas a pagar, contas a receber, lançamentos futuros, fluxo de caixa e domínio · `npm run web:test` — 201 unitários da árvore de permissões, dos perfis-semente contra a matriz do Anexo C, dos comandos de usuário e autenticação, do saldo derivado e da transferência, das nove regras de contas a pagar, das cinco de contas a receber e do fechamento de competência, das quatro de lançamentos futuros e das quatro do fluxo de caixa projetado, da aritmética de tiles, do parser de geocodificação e da entrega de arquivos |
| `apps/api` | API NestJS sobre PostgreSQL com RLS: contexto de tenant por transação, autorização negada por padrão, `problem+json`, idempotência, concorrência otimista, a entrada fiscal de compra, os clientes com visão 360 e crédito segregado do cadastro, os usuários e perfis com convite sem senha, os locais de operação, a autenticação com Argon2id, o worker de notificação com adaptador SMTP, as contas a pagar com aprovação por alçada, as contas a receber com fechamento de competência, o worker de conversão de lançamentos futuros e a projeção de caixa calculada uma vez para duas telas | `npm run api:test` — 254 testes contra PostgreSQL real · `verificar-rotas.mjs` — 110/110 rotas declaram autorização explícita, e o build reprova quem esquecer |
| `packages/contracts` | Esquemas Zod compartilhados entre API e clientes: primitivos, catálogo de erros, catálogo das 125 permissões, a árvore módulo → tela → ação derivada dele, entidades e a chave de acesso da NF-e com dígito verificador | Compilado no CI; consumido pelos dois lados — o front deixou de ter vocabulário próprio |
| `packages/db` | 22 migrações SQL: fundação, identidade, auditoria, equipamentos, contratos, RLS, outbox, geoespacial, idempotência, nota fiscal de compra, eixo de cliente, franquia e preço, consumo, proveniência da coordenada, autenticação, superfície fechada de autenticação, centro de custo e conta bancária, notificação, contas a pagar, contas a receber, lançamentos futuros com fluxo de caixa projetado, e o escopo de cliente que a restrição de coerência recusava | `npm run db:test` — 177 assertivas de invariante contra PostgreSQL real, com e sem PostGIS |
